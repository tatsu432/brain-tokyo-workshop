...
test_observations = [
    ...
    np.array([0.5, 0.1, 0.2, 0.0, 0.0, 0.8, 0.0, -0.5, -0.5, 0.1, 0.0, 0.0]),  # ball high
    np.array([-0.3, 0.1, -0.1, 0.0, -0.2, 0.3, 0.1, 0.0, 0.5, 0.1, 0.0, 0.0]), # moving left
]
...
print(f"Both outputs saturated: {sat_both}/{total_tests} ({100*sat_both/total_tests:.0f}%)")
print(f"One output saturated:   {sat_one}/{total_tests} ({100*sat_one/total_tests:.0f}%)")
print(f"No saturation:          {sat_none}/{total_tests} ({100*sat_none/total_tests:.0f}%)")
