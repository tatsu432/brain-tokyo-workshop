...
def _render_episode(
    ...
    print(f"Agent vs {'Archived Opponent' if opponent_data else 'Baseline'}", flush=True)
    ...
    try:
        for step in range(max_steps):
            ...
            if opponent_wVec is not None:
                ...
                opp_annOut = act(opponent_wVec, opponent_aVec, nInput, nOutput, opp_state)
...
class RenderManager:
    ...
    def should_render(self, generation: int) -> bool:
        ...
        if self._is_process_alive():
            print(f"[RenderManager] Skipping render at gen {generation}: previous render still running")
            return False
    ...
    def render_best(
        ...
        if final_opponent_data is None and use_random_archive_opponent and self._opponent_archive:
            import random
        ...
        print(f"[RenderManager] Started render process (PID: {self._current_process.pid}) for generation {generation}")
    ...
    def update_archive(self, archive: List[Tuple[np.ndarray, np.ndarray, float, int]]):
        ...
        """
        self._opponent_archive = [(w.copy(), a.copy(), f, g) for w, a, f, g in archive] if archive else []
...
def render_individual(
    ...
    if blocking:
        _render_episode(
            wVec, aVec, nInput, nOutput, actSelect,
            opponent_data, generation, max_steps, render_fps
        )
        return None
    else:
        # Use RenderManager for non-blocking
        manager = RenderManager(render_interval=1, max_steps=max_steps, render_fps=render_fps)
        manager.render_best(wVec, aVec, nInput, nOutput, actSelect, generation, opponent_data, False)
...
def _run_from_file(filepath: str):
    ...
    print(f"[RenderSubprocess] Starting render for generation {data['generation']}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--render-file', type=str, help='Path to render data file (for subprocess mode)')
