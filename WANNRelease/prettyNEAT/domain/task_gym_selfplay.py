...
class OpponentPolicy:
    ...
    def __init__(self, wVec: np.ndarray, aVec: np.ndarray, 
                 nInput: int, nOutput: int, actSelect: str):
        """
        Args:
...
class ArchivedIndividual:
    ...
    def to_policy(self, nInput: int, nOutput: int, actSelect: str) -> OpponentPolicy:
        ...
        return OpponentPolicy(
            self.wVec, self.aVec, nInput, nOutput, actSelect
        )
...
class SelfPlayGymTask:
    ...
    def add_to_archive(self, wVec: np.ndarray, aVec: np.ndarray, 
                       fitness: float, generation: int):
        """
    ...
    def get_archive_snapshot(self) -> List[Tuple[np.ndarray, np.ndarray, float, int]]:
        ...
        """
        return [(ind.wVec.copy(), ind.aVec.copy(), ind.fitness, ind.generation) 
                for ind in self.archive]
    
    def load_archive_snapshot(self, snapshot: List[Tuple[np.ndarray, np.ndarray, float, int]]):
        ...
        for i, (wVec, aVec, fitness, gen) in enumerate(snapshot):
            ...
            if verbose:
                ...
                if nan_count > 0 or inf_count > 0:
                    print(f"  [load_archive] WARNING: Archive individual {i} has {nan_count} NaN and {inf_count} inf values in wVec", flush=True)
            
            self.archive.append(ArchivedIndividual(wVec_clean, aVec_clean, fitness, gen))
    ...
    def _sample_archive_opponents(self, n: int) -> List[OpponentPolicy]:
        ...
        return [ind.to_policy(self.nInput, self.nOutput, self.actSelect) 
                for ind in sampled]
    ...
    def getFitness(
        ...
        if verbose or debug:
            print(f"  [getFitness] baseline_reps={baseline_reps}, archive_reps={archive_reps}, archive_size={len(self.archive)}", flush=True)
        
        # Evaluate against baseline
        for i in range(baseline_reps):
            if debug:
                print(f"  [getFitness] Starting baseline episode {i+1}/{baseline_reps}", flush=True)
        ...
        if archive_reps > 0 and self.archive:
            ...
            if debug:
                print(f"  [getFitness] Sampled {len(archive_opponents)} archive opponents for {archive_reps} reps", flush=True)
            
            for i, opponent in enumerate(archive_opponents):
                if debug:
                    print(f"  [getFitness] Starting archive episode {i+1}/{len(archive_opponents)}", flush=True)
    ...
    def _evaluate_episode(
        ...
        try:
            ...
            print(f"    [_evaluate_episode] ERROR in initial env.step(): {e}", flush=True)
            raise
        ...
        for tStep in range(self.maxEpisodeLength):
            ...
            if verbose and tStep > 0 and tStep % log_interval == 0:
                print(f"    [_evaluate_episode] Step {tStep}/{self.maxEpisodeLength}, reward so far: {totalReward:.2f}, opponent: {'archive' if opponent_policy else 'baseline'}", flush=True)
            ...
            try:
                ...
                print(f"    [_evaluate_episode] ERROR in env.step() at step {tStep}: {e}", flush=True)
                raise
            ...
            if done:
                if verbose and tStep > log_interval:
                    print(f"    [_evaluate_episode] Episode done at step {tStep}, total reward: {totalReward:.2f}", flush=True)
                break
