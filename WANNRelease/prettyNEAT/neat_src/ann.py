...
def getNodeOrder(
    nodeG: np.ndarray, connG: np.ndarray
) -> tuple[np.ndarray, np.ndarray] | tuple[bool, bool]:
    ...
    edge_in = np.sum(
        connMat, axis=0
    )  # an array of length N_nodes, where edge_in[i] is the number of incoming connections to node i index
